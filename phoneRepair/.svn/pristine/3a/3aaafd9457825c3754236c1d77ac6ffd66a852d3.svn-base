  ...
  <!--<![endif]-->
    ...
    <div class="content">
        ...
        <div class="header">
            ...
            <li><a href="/phoneRepair/user?method=getAllUser&currPage=1&pageSize=5"></a> <span class="divider"></span></li>
        </ul>
...
<div class="well">
    <table class="table">
      ...
      <c:forEach var="user" items="${pageBean.list }">
          ...
          <td>${user.score }</td>
          <td>
              <a href="/phoneRepair/user?method=showUser&uid=${user.uid}"><i class="icon-pencil"></i></a>
              <a href="#myModal${user.uid }" role="button" data-toggle="modal"><i class="icon-remove"></i></a>
          </td>
        </tr>
        <div class="modal small hide fade" id="myModal${user.uid }" tabindex="-1" role="dialog" aria-labelledby="myModalLabel" aria-hidden="true">
    ...
    <div class="modal-footer">
        ...
        <button id="${user.uid }" class="remove btn btn-danger" data-dismiss="modal">确定</button>
    </div>
</div>
      </c:forEach>  
      </tbody>
...
<div class="pagination" style="margin-left:300px;">
    <ul>
        <li><a href="/phoneRepair/user?method=getAllUser&currPage=1&pageSize=5">首&nbsp;&nbsp;&nbsp;页</a></li>
        <c:if test="${pageBean.currPage!=1 }">
        <li><a href="/phoneRepair/user?method=getAllUser&currPage=${pageBean.currPage-1 }&pageSize=5">上一页</a></li>
        </c:if>
        <c:if test="${pageBean.currPage==1 }">
        <li><a href="/phoneRepair/user?method=getAllUser&currPage=1&pageSize=5">上一页</a></li>
        </c:if>
        <li><a href="/phoneRepair/user?method=getAllUser&currPage=${pageBean.currPage }&pageSize=5">第&nbsp;${pageBean.currPage }&nbsp;页</a></li>
        <c:if test="${pageBean.currPage != pageBean.totalPage }">
        <li><a href="/phoneRepair/user?method=getAllUser&currPage=${pageBean.currPage+1 }&pageSize=5">下一页</a></li>
        </c:if>
        <c:if test="${pageBean.currPage == pageBean.totalPage }">
        <li><a href="/phoneRepair/user?method=getAllUser&currPage=${pageBean.totalPage }&pageSize=5">下一页</a></li>
        </c:if>
        <li><a href="/phoneRepair/user?method=getAllUser&currPage=${pageBean.totalPage }&pageSize=5">尾&nbsp;&nbsp;&nbsp;页</a></li>
    </ul>
</div>
        ...
        $(function() {
        	...
        	$(".remove").click(function(){
        		...
        		$.ajax({
        			...
        			data:"method=deleteUser&uid="+uid,
        			dataType:"json",
        			async:false,
            		error:function(){
            			alert("ajax出错");
