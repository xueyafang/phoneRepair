...
public class OrderDao extends BaseDao{
	...
	public List<List> showOrders(int uid, int currPage, int pageSize) throws SQLException {
		pstmt = con.prepareStatement("select oid,sname,date from orders,store where orders.sid=store.sid and uid=? limit ?,?");
		pstmt.setInt(1, uid);
	...
	public int totalCount(int uid) throws ClassNotFoundException, IllegalArgumentException, InvocationTargetException, IllegalAccessException, InstantiationException, SQLException, IOException {
		List<User> list = query("select oid,sname,date from orders,store where orders.sid=store.sid and uid=?",new Object[]{uid},User.class);
		return list.size();
	}

	public int orderCount() throws ClassNotFoundException, IllegalArgumentException, InvocationTargetException, IllegalAccessException, InstantiationException, SQLException, IOException {
		List<Orders>  list = query("select * from orders", new Object[]{}, Orders.class);
		return list.size();
	}
